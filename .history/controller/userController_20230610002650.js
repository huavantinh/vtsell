...
const login = async (req, res) => {
  ...
  let checkUserMail = await prisma.users.findFirst({ where: { email } });

  if (email) {
    if (checkUserMail.password) {
      return res.status(200).send(checkUserMail);
    }
    // console.log("oke 1 =>>>>>>", checkUserMail);
